  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button className="bg-primary text-primary-foreground" onClick={() => { setEditing(null); setOpen(true); }}>Add Account</Button>
      </div>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {accounts.map((a: any) => (
          <AccountCard key={a._id} account={a} onEdit={(acc) => { setEditing({ id: acc._id, ...acc }); setOpen(true); }} onDelete={(id) => del.mutate(id)} />
        ))}
      </div>
      <AccountForm open={open} onOpenChange={setOpen} initialValues={editing || undefined} />
    </div>
  );
}
