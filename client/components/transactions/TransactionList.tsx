    ...
    <div className="space-y-4">
        ...
        <div className="inline-flex rounded-md border p-1">
          {["all", "income", "expense", "transfer"].map((t) => (
            <button
              ...
              className={`px-3 py-1 text-sm capitalize rounded ${type === t ? "bg-primary text-primary-foreground" : "hover:bg-accent"}`}
            >
              {t}
            </button>
          ))}
      ...
      <ul className="divide-y rounded-xl border bg-white/50 backdrop-blur dark:bg-black/20">
        {data.map((t: any) => (
          <li
            key={t._id}
            className={`flex items-center gap-3 p-3 border-l-4 ${t.type === "income" ? "border-green-500" : t.type === "expense" ? "border-red-500" : "border-blue-500"}`}
          >
            ...
            <div className="flex-1">
              <div className="flex items-baseline justify-between">
                ...
                <span
                  className={
                    t.type === "income" ? "text-success" : "text-red-600"
                  }
                >
            ...
            <div>
              <Button
                ...
                size="sm"
                onClick={async () => {
                  await apiFetch(`/api/transactions/${t._id}`, {
                    method: "DELETE",
                  });
                  location.reload();
                }}
